...
#
# [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/brj0/mepylome/blob/main/examples/publication/4_cns_tumors.ipynb)
#
#
...
# %% language="bash"
#
# pip install mepylome ipython pillow ipywidgets kaleido==0.2.1
...
from mepylome import Manifest, clear_cache
...
datasets = {
    "cns_tumors": {
        "xlsx": "https://static-content.springer.com/esm/art%3A10.1038%2Fnature26000/MediaObjects/41586_2018_BFnature26000_MOESM3_ESM.xlsx",
        "idat": ["GSE109381"],
    },
}
...
# [Capper et al. (2018)](https://doi.org/10.1038/nature26000). These
...
# ## 1. Data Loading
#
# This section replicates the methylation analysis performed in [Capper et
# al. (2018)](https://doi.org/10.1038/nature26000). To begin, we
# download the required data and organize it within the designated directories.

# %%
# Initialize directories.
tumor_site = "cns_tumors"
...
if not (excel_path := analysis_dir / f"{tumor_site}.xlsx").exists():
    ...
    # Deletes the first row (useless description).
    pd.read_excel(excel_path, skiprows=1).to_excel(excel_path, index=False)
...
analysis = MethylAnalysis(
    ...
    n_cpgs=25000,
    load_full_betas=False,  # Uses less memory (necessary on Google Colab)
    overlap=True,
...
# %%
# Calculate UMAP
analysis.idat_handler.selected_columns = ["Reference Group abbreviation"]
analysis.make_umap()
...
# %%
# Set CV folds and filter out samples with labels below minimum count
analysis.idat_handler.selected_columns = ["Reference Group abbreviation"]
...
print(f"Cross-validation folds set to: {analysis.cv_default}")
print(f"Labels dropped due to insufficient samples (< {analysis.cv_default}):")
...
# %%
# Save CNV example
analysis.make_cnv_plot("9647455008_R06C01")
...
cn_summary_path = calculate_cn_summary(
    analysis, "Reference Group abbreviation"
)
