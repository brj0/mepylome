...
def _read_tcga_clinical(
    ...
    clinical_df = pd.read_csv(Path(metadata_clinical).expanduser(), sep="\t")
    # TCGA changed case_id to cases.case_id
    if "case_id" not in clinical_df.columns:
        if "cases.case_id" not in clinical_df.columns:
            raise KeyError(
                "Neither 'case_id' nor 'cases.case_id' found in clinical TSV."
            )
        clinical_df = clinical_df.rename(columns={"cases.case_id": "case_id"})
    return clinical_df
...
def make_tcga_metadata(
    ...
    if (
        include_clinical
        and annotation_csv_path.exists()
    ...
    if annotation_csv_path.exists():
        logger.info(
            "Rebuilt %s. Keeping existing annotation: %s",
            manifest_path.name,
            annotation_csv_path,
        )
        return
