...
def find_genes_within_bins(
    ...
    detail_Range = [
        ...
        for start, end in zip(
            detail_Start_bin, detail_End_bin + 1, strict=True
        )
    ]
...
def read_cnv_data_from_disk(
    ...
    results: dict[str, pd.DataFrame | None] = dict(
        zip(extract, unzipped, strict=True)
    )
...
class CNVPlot:
    ...
    def __init__(
        ...
        cnv_dir: str | Path,
        cnv_file: str,
        genes: list[str] = CONFIG["genes"]["default_genes_list"],
        host: str = "localhost",
        port: int = 8050,
    ) -> None:
    ...
    def get_app(self) -> Any:
        ...
        def update_graph(
            genes_sel: list[str],
            url_path: str,
        ) -> tuple[go.Figure | Any, pd.DataFrame | Any, str]:
