...
class ProbeAddress(Enum):
    """AddressA_ID and AddressB_ID are columns in the manifest csv that contain
    internal Illumina probe identifiers.

    Type II probes use AddressA_ID; Type I uses both, because there are two
    probes, two colors.

    probe intensities in .idat files are keyed to one of these ids, but
    processed data is always keyed to the IlmnID probe "names" -- so this is
    used in converting between IDs. It is used to define probe sets below in
    this probes.py
    """
...
# == IG [AddressA] + II[red] + IR[AddressA]
...
""" DISABLED because these faster to process with normals then remove during
postprocessing.
