...
# f =  open(file_path, 'rb')
# file_type = f.read(4).decode('utf-8')
# _idat_version = f.read(8)
# idat_version = int.from_bytes(_idat_version, byteorder='little', signed=True)
# _num_fields = f.read(4)
# num_fields = int.from_bytes(_num_fields, byteorder='little', signed=True)
# offsets = {}
# for _idx in range(num_fields):
    # _key = f.read(2)
    # key = int.from_bytes(_key, byteorder='little', signed=False)
    # _offsets = f.read(8)
    # offsets[key] = int.from_bytes(_offsets, byteorder='little', signed=True)
# f.close()


idat_file =  open(file_path, 'rb')
file_type = read_char(idat_file, 4)
...
def seek_to_section(section_code):
    offset = offsets[section_code.value]
    idat_file.seek(offset)

seek_to_section(IdatSectionCode.BARCODE)
barcode = read_string(idat_file)

seek_to_section(IdatSectionCode.CHIP_TYPE)
chip_type = read_string(idat_file)

seek_to_section(IdatSectionCode.NUM_SNPS_READ)
n_snps_read = read_int(idat_file)

seek_to_section(IdatSectionCode.NUM_BEADS)
n_beads = npread(idat_file, '<u1', n_snps_read) # was <u1

seek_to_section(IdatSectionCode.ILLUMINA_ID)
illumina_ids = npread(idat_file, '<i4', n_snps_read)

seek_to_section(IdatSectionCode.MEAN)
probe_means = npread(idat_file, '<u2', n_snps_read) # '<u2' reads data as numpy unsigned-float16
...
grn = methylprep.models.Channel.GREEN
red = methylprep.models.Channel.RED


idat_grn = IdatDataset(file_path, grn)
