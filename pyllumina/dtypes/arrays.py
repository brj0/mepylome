...
class ArrayType(Enum):
    ...
    def from_probe_count(cls, probe_count):
        ...
        if (
            315000 <= probe_count <= 362000
        ):  # V1 actual count from idat: 315639
            return cls.ILLUMINA_MOUSE
            # B1 V1 274390 actual probes == rows in manifest
            # B3 V2 299344 actual probes == rows in manifest file; 361821 count
            # from idat
            # mm295 v2 ??s

        if 56000 <= probe_count <= 1100000:
            LOGGER.warning(
                f"Probe count ({probe_count}) falls outside of normal range. "
                "Setting to newest array type: EPIC"
            )
            return cls.ILLUMINA_EPIC

        raise ValueError(
            f"Unknown array type: ({probe_count} probes detected)"
        )
    ...
    def num_probes(self):
        ...
        probe_counts = {
            ...
            # NOTE: if EPIC+ is not set to 868699, noob fails downstream. but
            # there are only 868698 probes by my count.
            # ArrayType.ILLUMINA_MOUSE: 268833, #274390 rows in manifest RND1
            # on 2020-03-25.
            # this includes all types. so ch+cg types == 268832
            # test: added +1 because mouse controls were short by one. and this
            # fixed it. prob
            # need to test them ALL and add +1 header in manifest.py code.
            # B1 V2 = 273757, # test: list where all control probes start
            # ArrayType.ILLUMINA_MOUSE: 298710, #B3 V2 row number for first
            # control probe (after [Controls],,,,,,header)
            # ArrayType.ILLUMINA_MOUSE: 297414, C20_V4
            # 287054 #287054 is first control row; no header row
            # 297415 # row number for first control probe (after header
            # [Controls],,,, ) with row count starting at zero.
            # 292585, # MM285_V1 | sesame's qualityMask had 293199 probes |
            # control was 635 probes
        }
    ...
    def num_controls(self):
        probe_counts = {
            ...
            # 1966 controls in B3, and in sesame's manifest, but not in
            # MM285_v2 or v3.
    ...
    def num_snps(self):
        probe_counts = {
            ...
            # 1353, #in v2: 536, #was at end of file, now before control
            # (testing)
